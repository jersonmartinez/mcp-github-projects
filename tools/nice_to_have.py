...
(`tools.nice_to_have`) keep resolving. New code MUST import from `tools.meta.nice_to_have`.
See docs/architecture/PROJECT_STRUCTURE.md §1 (compatibility layer).
"""
import tools.meta.nice_to_have as _canon  # noqa: E402

# Re-export every module-level name (including non-__all__ symbols such as
# imported client classes that tests patch) so `patch('tools.nice_to_have.X')` resolves
