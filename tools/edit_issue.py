...
(`tools.edit_issue`) keep resolving. New code MUST import from `tools.issues.edit_issue`.
See docs/architecture/PROJECT_STRUCTURE.md §1 (compatibility layer).
"""
import tools.issues.edit_issue as _canon  # noqa: E402

# Re-export every module-level name (including non-__all__ symbols such as
# imported client classes that tests patch) so `patch('tools.edit_issue.X')` resolves
