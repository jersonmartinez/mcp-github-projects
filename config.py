...
(`config`) keep resolving. New code MUST import from `core.config`.
See docs/architecture/PROJECT_STRUCTURE.md §1 (compatibility layer).
"""
import core.config as _canon  # noqa: E402

# Re-export every module-level name (including non-__all__ symbols such as
# imported client classes that tests patch) so `patch('config.X')` resolves
